...
float4 ps_main(VERTEX_OUTPUT Input) : COLOR
{
	...
	if(alpha > 0)
	{
		...
		//INSIDE PORTAL
		//===================================================================================
		float4 portalcolor = tex2D(PortalSampler,Input.TexCoord);
		float4 portalglow = tex2D(PortalGlowSampler, Input.TexCoord);

		//----------------------------------------------------
		//BLUR THE PORTAL GLOW
		//----------------------------------------------------
		if((portalglow.x == 0) && (portalglow.y == 0) && (portalglow.z == 0)) //if not doing mesh glow
		{
			float factor = 1.0/(4.0*3.0); //(tex lookups * loops)
			float spread = invdepth*6;
			float4 coord;
			...
			{
				o = offset*v;
				coord = tex2D(PortalGlowSampler, Input.TexCoord + float2(o,0));
				if(coord.a <= depth){ portalglow += (coord*factor); }
				coord = tex2D(PortalGlowSampler, Input.TexCoord + float2(-o,0));
				if(coord.a <= depth){ portalglow += (coord*factor); }
				coord = tex2D(PortalGlowSampler, Input.TexCoord + float2(0,o));
				if(coord.a <= depth){ portalglow += (coord*factor); }
				coord = tex2D(PortalGlowSampler, Input.TexCoord + float2(0,-o));
				if(coord.a <= depth){ portalglow += (coord*factor); }
				v += spread;
			}
			portalglow *= 1.5f; //halo portalglow intensity
		}
		else
		{
			portalglow *= 0.4f; //model portalglow intensity
		}

		//SET THE COLOR
		color = (color*(1-alpha))+(portalcolor*alpha);
		color += (glow+portalglow);
