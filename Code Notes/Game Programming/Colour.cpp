...
BUMP MAPPING: 
• Changes interpolated normal of vertex for pixel according to height map/grayscale image
...
MAP CREATION:
• Rays are cast from each low-res vertex in the direction of that vertex's interpolated normal
• Ray is intersected against the high-res model to find the point of intersection
• The normal at the point of intersection is the stored in the normal map
• The position of the stored normal in the normal map is 
  determined by the low-res model's UV coordinate
