...
// CONCEPTS
// Restricts T to be integer or floating point
template <typename T> concept MyConcept = std::integral<T> || std::floating_point<T>;
template <typename T> requires MyConcept<T> auto fn(T a, T b) { return a + b; }
template <typename T1, typename T2> requires MyConcept<T1> && MyConcept<T2> auto fn(T1 a, T2 b) { return a + b; }

// INLINE CONCEPTS
template <typename T> requires std::integral<T> || std::floating_point<T>
auto fn(T a, T b) { return a+b; }
